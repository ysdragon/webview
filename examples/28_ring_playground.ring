...
func load_html()
	...
	<head>
		...
		<style>
			...
			.toolbar h1 { font-size: 1.5em; margin: 0; color: var(--text-primary); }
			.toolbar h1 i { color: var(--accent-cyan); margin-right: 0.5em; }
			...
			.panels {
				display: flex; flex-grow: 1; gap: 25px;
			...
			.cm-s-material-darker.CodeMirror { background-color: transparent; }
			.CodeMirror-lines { padding: 10px 0; }
			.CodeMirror-scroll { padding-bottom: 10px; }
			#output {
				flex-grow: 1;
				padding: 1.2em;
				margin: 0;
	...
	<body>
		...
		<script>
			...
			CodeMirror.defineSimpleMode("ring", {
			  start: [
				{regex: /"(?:[^\\]|\\.)*?(?:"|$)/, token: "string"},
				{regex: /(?:#|\/\/).*$/, token: "comment"},
				{regex: /\/\*/, token: "comment", next: "comment_multiline"}, // Multi-line comment start
				{regex: /(?:do|bye|and|but|again|call|case|catch|class|def|done|else|elseif|end|exit|for|foreach|from|func|get|give|if|import|in|load|loop|new|next|not|off|ok|on|or|other|package|private|put|return|see|step|switch|to|try|while|endfunc|endclass|endpackage|endif|endfor|endwhile|endswitch|endtry|function|endfunction|break|continue)\b/, token: "keyword"},
				{regex: /true|false|null|nl/, token: "atom"},
			  ...
			  comment_multiline: [
				{regex: /.*?\*\//, token: "comment", next: "start"}, // Multi-line comment end
				{regex: /.*/, token: "comment"} // Inside multi-line comment
			  ]
			});
