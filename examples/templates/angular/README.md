# Ring WebView + Angular Template

Angular (TypeScript) frontend bundled to a single HTML file with Vite +
`vite-plugin-singlefile`, loaded by a Ring WebView backend.

> This template uses Vite with `@analogjs/vite-plugin-angular` (standalone
> component, zoneless change detection) rather than the Angular CLI.
> The plugin runs with `fastCompile: true` — on rolldown-based Vite 8 this is
> required for AOT template compilation (without it the app fails at runtime
> with `Error: JIT compiler unavailable`).
...
- The Angular app calls `window.increment()` / `window.decrement()` (each
